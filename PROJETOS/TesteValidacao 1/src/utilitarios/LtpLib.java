...
//		 ----------------------------------------------------------;
   ...
   public static GregorianCalendar[] datasVencimento ( int  numParc , GregorianCalendar  dataBase ){
	   ...
	   for (int i=0; i<numParc;i++) {
		   ...
		   data.add(GregorianCalendar.DAY_OF_MONTH, (i+1)*30);
   ...
   public static GregorianCalendar[] domingosAno(int ano) {
	   GregorianCalendar data = new GregorianCalendar(ano,0,1);
	   int numDom = 0;
	   while (ano==data.get(GregorianCalendar.YEAR)) {
		   if (data.get(GregorianCalendar.DAY_OF_WEEK)==1) numDom++;
		   data.add(GregorianCalendar.DAY_OF_MONTH, 1);
	   }
	   GregorianCalendar[] resposta = new GregorianCalendar[numDom];
	   data.set(ano, 0, 1);
	   int i=0;
	   while (ano==data.get(GregorianCalendar.YEAR)) {
		   if (data.get(GregorianCalendar.DAY_OF_WEEK)==1) {
			   resposta[i] = (GregorianCalendar)data.clone();
			   i++;
		   }
		   data.add(GregorianCalendar.DAY_OF_MONTH, 1);
	   }
